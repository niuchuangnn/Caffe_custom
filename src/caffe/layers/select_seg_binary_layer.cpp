...
namespace caffe {
    ...
    void SelectSegBinaryLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
        ...
        const int label_type = this->layer_param_.select_seg_binary_param().label_type();
...
//        label_dim_ = this->layer_param_.window_cls_data_param().label_dim();
        label_dim_ = this->layer_param_.select_seg_binary_param().label_dim();
        ...
        while (std::getline(infile, linestr)) {
            ...
            iss >> imgfn;
            string segfn = "";
            if (label_type != SelectSegBinaryParameter_LabelType_NONE) {
                iss >> segfn;
            }
            SEGITEMS item;
            item.imgfn = imgfn;
...
//            this->prefetch_label_.Reshape(batch_size, 1, crop_size, crop_size);
            for (int i = 0; i < this->prefetch_.size(); ++i) {
                this->prefetch_[i]->dim_.Reshape(batch_size, label_dim_, 1, 1);
            }
...
// This function is used to create a thread that prefetches the data.
    template <typename Dtype>
    void SelectSegBinaryLayer<Dtype>::load_batch(BatchDim<Dtype> *batch){
...
//        Dtype* top_cls_label = this->prefetch_data_dim_.mutable_cpu_data();
        ...
        Dtype* prefetch_cls_label = batch->dim_.mutable_cpu_data();
        ...
        const int label_type = this->layer_param_.select_seg_binary_param().label_type();
        const int ignore_label = this->layer_param_.select_seg_binary_param().ignore_label();
...
//                                      0, 0, is_color, &img_row, &img_col);
            ...
            if (label_type == SelectSegBinaryParameter_LabelType_PIXEL) {
            ...
            else if (label_type == SelectSegBinaryParameter_LabelType_IMAGE) {
            ...
            // class label
            offset = batch->dim_.offset(item_id);
